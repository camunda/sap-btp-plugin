...
import SelectList from "sap/m/Select"
import Text from "sap/m/Text"
import uid from "sap/base/util/uid"
import VBox from "sap/m/VBox"
import Event from "sap/ui/base/Event"
import { BPMNformData, Component, ControlType, GeneratedControl, userFormData, SelectionModes } from "./BPMNformData"
...
class BPMNform extends Control {
  metadata = {
    properties: {
      ...
      formStep: { type: "number", bindable: true }
    },
    aggregations: {
      items: { type: "sap.ui.core.Control", multiple: true }
    },
    events: {
      // fired, when form has been filled completly and summary is shown
      summary: {},
  ...
  getValueFromControl(type: ControlType, control: Control): boolean | string | string[] {
    ...
    switch (type) {
      ...
      case ControlType.Radio:
        ...
        if (selectedButton) {
          value = selectedButton.getCustomData()[0].getKey() as string
        } else {
          value = ""
        }
        break
      case ControlType.CheckBox:
        if (control && (control as CheckBox).getVisible()) {
          value = (control as CheckBox).getSelected() as boolean
        } else {
          value = ""
        }
        break
      case ControlType.Text:
        break
      default:
        console.error(`Error 1650474606: working an unknown form control type ${type}`)
  ...
  private addTextfield(element: Component): Control {
    ...
    const control = new Input(this._generateControlId(element), {
      ...
      liveChange: (event) => {
        ...
        this._setValueState(control, element, event.getParameter("value") as string)
      }
    })
  ...
  private addDynamicSumAutomatic(element: Component): Control {
    ...
    const control = new MultiComboBox(this._generateControlId(element), {
      ...
      selectionChange: (event) => {
        this._provideValueToView(element, control)
        this._setValueState(control, element, (event.getSource() as MultiComboBox).getSelectedKeys().length)
      }
    })
  ...
  private addSuggestInput(element: Component): Control {
  ...
  init(): void {
    ...
    sap.ui
      .getCore()
      .getEventBus()
      .subscribe("Camunda", "startProcess", () => {
        this._initLocalModel()
      })
  }
  ...
  /**
   * update a bdaas basket position with the questions and answers provided in the bdaas-run
   * (which subsequently also updates the history entry for that basket position with the QAs
   * via the UPDATE hook in the srv layer)
   *
   * @param id the basket identifier
   * @param subId supposedly a GUID, doing string here
   */
  async _updateBasket(id: string, subId: string) {
    if (!subId) {
      Log.error("Error 1673272841617: No basket id given, so update is not possible")
    }
    const historyModel = sap.ui.core.Component.getOwnerComponentFor(this).getModel("HistoryView") as JSONModel
    const qaData = historyModel.getData().qa
    // patch the stored bdaas run with the QAs
    // triggering an update on the "history" entry for that basketposition as well
    // (the trigger lives in the srv layer)
    // be defensive w/ try-catch as to not stop the overall process-/data-flow
    try {
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
      const response = await fetch(`/rt/BasketPositions(${subId})`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          qaBlob: JSON.stringify(qaData)
        })
      })
      if (response.ok) {
        Log.info(`//> (basket ${id}) updated basket position ${subId} w/ QAs!`)
      } else {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        const body = await response.json()
        Log.error(`//> Error 1659457442658`)
        Log.error(`//> (basket ${id}) updating basket position ${subId} failed:`)
        Log.error(`//> ${JSON.stringify(body)}`)
      }
    } catch (err) {
      Log.error(`//> Error 1659510273767`)
      Log.error(`//> (basket ${id}) updating basket position ${subId} failed:`)
  ...
  endProcess(data: WebSocketData): void {
    ...
    // intentional async side-effect!
    void this._updateBasket(data.basket.id, data.basket.subId)
    ...
    // prep q+a container
    const answers = new VBox({
      width: "100%"
    })
    answers.addStyleClass("sapUiResponsiveMargin")
    // add heading to the q+a container
    if (data.type === "final-task-success") {
      answers.addItem(new Title({ text: this.getFinalResultTextSuccess(), level: "H1", wrapping: true }))
    }
    if (data.type === "final-task-fail") {
      ...
      answers.addItem(title)
    }

    const viewModel = sap.ui.core.Component.getOwnerComponentFor(this).getModel("AppView") as JSONModel

    // these are the questions and answers that the "final task" handler populates
    data.varsToShow.forEach((tupel: { label: string; value: string; modelKey: string }) => {
      viewModel.setProperty(`/${tupel.modelKey}`, tupel.value)
      const outputContainer = new HBox()
      outputContainer.addStyleClass("sapUiResponsiveMargin")
      if (tupel.modelKey === "historyLink") {
        const history = new Button({
          // text: "Historie der Warenkorbposition",
          text: this.getModel("i18n").getResourceBundle().getText("history_button") as string,
          press: () => {
            window.open(tupel.value)
          }
        })
        history.addStyleClass("sapUiSizeCompact")
        outputContainer.addItem(history)
      } else {
        outputContainer.addItem(new Text({ text: `${tupel.label}: `, renderWhitespace: true }))

        outputContainer.addItem(
          new Text(`${uid()}-label_for_${tupel.modelKey}`, { text: tupel.value, renderWhitespace: true }).addStyleClass(
            "sapUiSmallMarginBegin"
          )
        )
        if (tupel.modelKey === "coElement") {
          const copyBtn = new Button({
            icon: "sap-icon://copy",
            press: () => {
              Clipboard.copyTextToClipboard(tupel.value)
            }
          })
          copyBtn.addStyleClass("sapUiSmallMarginBegin")
          outputContainer.addItem(copyBtn)
        }
      }
      answers.addItem(outputContainer)
    })
    container.addItem(answers)
    container.data("controlType", ControlType.Summary)
    container.data("varsToShow", data.varsToShow)
  ...
  getHistoricBasketPositionId(): string {
    return this.getLocalModel().getProperty("/BPMNform/bdaasHistoricBasketPositionId") as string
  }
  ...
  renderer = {
     ...
     */
    render: (rm: RenderManager, bpmnForm: Control): void => {
      console.debug(`[${bpmnForm.getMetadata().getName()}] > rendering`)
      rm.openStart("div", bpmnForm)
      rm.style("width", "35rem")
      rm.openEnd()
      if (bpmnForm.getAggregation("items")) {
        // eslint-disable-next-line @typescript-eslint/no-extra-semi
        ;(bpmnForm.getAggregation("items") as Control[]).forEach((control: Control) => {
